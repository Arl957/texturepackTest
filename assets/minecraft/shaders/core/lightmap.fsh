...
void main() {
    ...
    vec3 color = vec3(
        block_brightness,
        block_brightness * ((block_brightness * 0.6 + 0.4) * 0.6 + 0.6),
        block_brightness * (block_brightness * block_brightness * 0.6 + 0.9)
    );

    color = mix(color, lightmapInfo.AmbientColor, lightmapInfo.AmbientLightFactor);

    color += lightmapInfo.SkyLightColor * (sky_brightness - 0.3);          // make sky light less effective, pitch black at night
    ...
    color = smoothstep(0.0, 1.0, color);            // smoothing light
    if (lightmapInfo.BrightnessFactor > 1.0) {     // anti fullbright
